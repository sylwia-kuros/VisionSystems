...
for i in range(ROI_start, ROI_end, ANALYSIS_STEP):  # start, stop, step
    ...
    # conservative approach - update only pixels classified as background
    if 'blurred_image' in globals():
        background_model = np.where(blurred_image == 0, background_model, prev_background_model)

...
for i in range(ROI_start, ROI_end, ANALYSIS_STEP):  # start, stop, step
    ...
    if prev_background_model.any() < image_gray.any():
        background_model = prev_background_model + 1
    elif prev_background_model.any() > image_gray.any():
        background_model = prev_background_model - 1
    else:
        background_model = prev_background_model

    absolute_difference = cv2.absdiff(background_model * 1.0, image_gray)
